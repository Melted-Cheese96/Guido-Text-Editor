...
class app():
    ...
    def openFile(self): #Open's a file that the user has pressed
        self.app.filename = filedialog.askopenfilename(filetypes=[('Text files', '*')])
    ...
    def searchForWord(self):
        items = self.textWidget.get('1.0', 'end-1c')
        words = items.split(' ')
        for item in words:
            if item == self.searchEntry.get():
                search = self.textWidget.search(item, '1.0', 'end-1c')
                print(search)
                self.textWidget.mark_set('insert', search)
    ...
    def changeRed(self):
        #self.textWidget.config(background='red')
        self.mainMenu.config(background='red')
        self.scrollbar.config(background='red')

    def changeBlue(self):
        #self.textWidget.config(background='blue')
        self.mainMenu.config(background='blue')
        self.scrollbar.config(background='blue')

    def default(self):
        #self.textWidget.config(background='gainsboro')
        self.mainMenu.config(background='gainsboro')
        self.scrollbar.config(background='gainsboro')
    ...
    def stickyNotes(self):  #This function is responsible for setting up the text editor
        ...
        self.submenu4 = tk.Menu(self.mainMenu)
        self.mainMenu.add_cascade(menu=self.submenu4, label='Personalize')
        self.submenu4.add_command(label='Red', command=self.changeRed)
        self.submenu4.add_command(label='Blue', command=self.changeBlue)
        self.submenu4.add_command(label='Default', command=self.default)
